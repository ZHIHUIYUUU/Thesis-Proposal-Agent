...
export default function LiteraturePage() {
  ...
  async function runSearch() {
    ...
    try {
      const result = await searchOpenAlex(deepDive.searchPlan.openAlexUrl);
      dispatch({
        type: "SET_LITERATURE",
        payload: {
          ...result,
          query: deepDive.searchPlan.openAlexQuery,
        },
      });
    } catch (error) {
  ...
  return (
    <section className="page-grid">
      <div className="page-card">
        ...
        <h2>联网文献检索</h2>
        <p className="page-lead">这里检索的是公开文献元数据。它能提供 DOI、来源和开放链接，但最终缺口仍要靠人工阅读核验。</p>
        ...
        <div className="matrix-explainer">
          ...
          <p>
            先在下方至少勾选 2 篇文献。下一页会把这些文献当作证据，生成几个可改进的研究方向和理由；你选择一个方向后，系统会形成可继续修改的缺口草案。
          </p>
        </div>
        ...
        <div className="table-wrap">
          ...
          {!literature.works.length && (
            <p className="empty-text">
              {literature.searched
                ? "已完成检索，但当前查询没有返回可展示文献。可以返回深挖页收窄问题，或保留方向后换一个研究角度。"
                : "还没有检索结果。点击上方按钮开始检索。"}
            </p>
          )}
        </div>
